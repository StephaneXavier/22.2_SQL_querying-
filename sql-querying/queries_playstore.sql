...
SELECT app_name FROM analytics WHERE min_installs <= 50 AND rating > 0 ORDER BY
 rating desc;
...
SELECT app_name, reviews, price FROM analytics WHERE price >= 0.1 AND price <=1 ORDER BY reviews desc LIMIT 10;
...
-- at least 100,000 times. Display the name of the app along with the number of reviews, the min_installs, and the 
-- proportion.
